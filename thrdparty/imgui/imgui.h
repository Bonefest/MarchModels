...
// CUSTOM_PATCH: Including icons codepoints by default
#include "imgui_icons_kenney.h"
// END CUSTOM_PATCH
...
namespace ImGui
{
    ...
    // BEGIN CUSTOM_PATCH
    IMGUI_API void          TextColored(const char* fmt, ...)                               IM_FMTARGS(1);
    IMGUI_API void          TextColoredV(const char* fmt, va_list args)                     IM_FMTLIST(1);    
    // END CUSTOM_PATCH
...
enum ImGuiSliderFlags_
{
    ...
    // BEGIN CUSTOM_PATCH: 
    ImGuiSliderFlags_MultiRange             = 1 << 8,       // Whether to use different ranges for different sliders
    // END CUSTOM_PATCH
...
struct ImColor
{
    ...
    // BEGIN CUSTOM_PATCH: We need to add it here since otherwise it uses a wrong conversion (ImU32 to be precise)
